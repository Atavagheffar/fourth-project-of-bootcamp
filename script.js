...
// ************************************
document.getElementById("weather-info").addEventListener("click", myDropmenu);

function myDropmenu() {
  // console.log("ata");
  let crg = document.querySelector(".city-radio-group");
  // crg.style.display = "flex";

  if (crg.style.display === "flex") {
...
document.querySelectorAll('input[name="city"]').forEach((radio) => {
  radio.addEventListener("change", () => {
    ...
    let navcity = document.getElementById("navCityname");
    navcity.innerHTML = city;
    myDropmenu();
    fetchWeather(city);
  });
});
...
window.addEventListener("load", function () {
  document.getElementById("navCityname").innerHTML = "تهران";
  fetchWeather("Tehran");
});
